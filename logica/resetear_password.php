...
function reset_pass($token,$password){
    ...
    $sql = "SELECT * FROM reset_password R INNER JOIN usuarios U ON R.user_reset = U.id_usuario WHERE token = '$token'";
    ...
    if($data = $proceso->fetch_assoc()){
        ...
        if($data['fecha_reset'] == $hoy){
            $pos1 = stripos($data['nombres'], $password);
            $pos2 = stripos($data['apellidos'], $password);
            $pos4 = stripos($data['email'], $password);
            $pos5 = stripos($password, $data['nombres']);
            $pos6 = stripos($password, $data['apellidos']);
            ...
            || $pos5 !== false || $pos6 !== false || $pos8 !== false)
                ...
                $change = "UPDATE usuarios SET password='$hash' WHERE id_usuario = $id";
